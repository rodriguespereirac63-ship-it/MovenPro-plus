...
function atualizarMetricas(){
  document.getElementById('cozAguardando').textContent = pedidosAtuais.filter(p=>p.status==='aguardando' && itensDoSetor(p,'bar').length).length;
  document.getElementById('cozPreparo').textContent = pedidosAtuais.filter(p=>p.status==='preparo' && itensDoSetor(p,'bar').length).length;
  document.getElementById('cozProntos').textContent = pedidosAtuais.filter(p=>p.status==='pronto' && itensDoSetor(p,'bar').length).length;
  document.getElementById('cozMesas').textContent = mesasAtuais.filter(m=>m.status!=='livre').length;
}

function cardPedido(p){
  return `<article class="cozinha-card bar-card">
    <div class="cozinha-card bar-card-head">
      <strong>Mesa ${p.mesaNumero}</strong>
      <span>${new Date(p.data).toLocaleTimeString('pt-BR')}</span>
    </div>
    <p><b>Garçom:</b> ${p.garcom || '-'}</p>
    <div class="finance-list">
      ${itensDoSetor(p,'bar').map(i=>`<div><span>${i.qtd}x ${i.nome}</span><strong>${brl(i.total)}</strong><small>${p.obs || ''}</small></div>`).join('')}
    </div>
...
function render(){
  const visiveis = pedidosAtuais.filter(p=>p.status !== 'cancelado' && p.status !== 'entregue' && itensDoSetor(p,'bar').length);
  atualizarMetricas();
...
function escutarPedidos(){
  onSnapshot(collection(db, 'pedidosRestaurante'), (snap)=>{
    ...
    document.getElementById('listaAguardando').innerHTML = '<p>Erro ao carregar pedidos do bar. Confira Firestore Rules.</p>';
  });
...
document.getElementById('btnAtualizarBar')?.addEventListener('click', render);
document.getElementById('btnTelaCheiaBar')?.addEventListener('click',()=>{
